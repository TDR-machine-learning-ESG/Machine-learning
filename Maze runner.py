...
class Paddle:
    ...
    def __init__(self, x = 400,y=550, xspeed = 0,yspeed=0, coefs = 0, intercepts = 0):
        self.x = x
        self.y = y
        self.ylast = y-yspeed
        self.xlast = x-xspeed
    ...
    def draw(self):
        if self.winner == False:
            pygame.draw.rect(screen,BLACK,[self.x,self.y,100,20])
            pygame.draw.rect(screen,RED,[self.x+2,self.y,100-4,20-4])
        else:
            pygame.draw.rect(screen,BLACK,[self.x,self.y,100,20])
            pygame.draw.rect(screen,BLUE,[self.x+2,self.y,100-4,20-4])
...
class Ball:
    ...
    def update(self, paddle):
        ...
        elif self.x>paddle.x and self.x<paddle.x+100 and self.ylast<paddle.y-35 and self.y>=paddle.y-35:
            self.yspeed = self.yspeed * -1
...
def displayNetwork(layer_sctructure, coefs = testCoefs, command = 0):
    ...
    inputs = ["paddle x", "paddle y","ball x", "ball y", "ball Xspeed", "ball Yspeed"]
...
while not done:
    ...
    for i, paddle in enumerate(paddles):
        ...
        input = np.array([[paddle.x,paddle.y, balls[i].x, balls[i].y, balls[i].xspeed,balls[i].yspeed]])
        ...
        if paddle.alive == True:
            paddle.update() #poner: (cuarto_actual.pared_lista)
